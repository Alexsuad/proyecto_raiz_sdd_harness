...
- **Gates y scripts**: Scripts de validación automática u operacional para resguardar la calidad del software.
...
- **Progress e history**: Registro histórico de avances, tareas (como `task.md`) y bitácoras del desarrollo.
...
| `GEMINI.md` | Adaptador operativo para Gemini | Contexto mínimo, restricciones operativas y referencias hacia la constitución, el mapa documental y los workflows oficiales. | Reglas de estilo de código general, arquitectura del software. |
| `.agent/skills/` | Habilidades agénticas | Scripts de soporte, esquemas de entrada/salida y contratos operativos para tareas automatizadas específicas. | Datos de usuario confidenciales, código principal de la aplicación. |
| `.agent/workflows/` | Flujos de trabajo agénticos | Definiciones secuenciales de procesos multi-agente, gates de validación y control de transiciones. | Implementaciones directas de lógica de negocio. |
| `.agent/rules/` | Reglas de comportamiento local | Configuraciones de comportamiento local de herramientas y restricciones agénticas por subcarpeta. | Guías de usuario final. |
...
| `progress/` | Seguimiento de avance | Tableros de tareas (`task.md`), bitácoras de cambios menores, estados de sprints y control diario. | Documentos maestros de diseño técnico o código. |
| `scripts/` | Herramientas operativas locales | Scripts de verificación, utilidades de compilación, linters locales y helpers deterministas. | Lógica core de la aplicación final. |
...
- **Pendiente**: Validar contra el documento maestro original y realizar ajustes finos tras la primera prueba real del arnés de desarrollo.
